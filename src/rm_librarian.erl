...
check_in_resource([]) ->
    throw(no_resource);
check_in_resource(Conversation) ->
...
update_available_resources(Connection, Total, Resources) ->
    ...
    {Connection, Total, rm_store:find({Connection, available_resources})}.
...
check_in_needs_a_conversation_test() ->
    ?assertThrow(no_resource, check_in_resource([])).
...
check_out_resource_decreases_count(Connection) ->
    ?assertEqual({Connection, 1, 0}, check_out_resource(Connection, "id")).
...
check_in_resource_increases_count(Connection) ->
    ?assertEqual({Connection, 1, 1}, check_in_resource("id")).

check_in_resource_needs_id_already_checked_out(_) ->
    ?assertThrow(no_resource, check_in_resource("id")).
